package org.l2junity.loginserver.service;
...
import org.l2junity.loginserver.manager.SessionManager;
...
public class LoginServerRMI extends UnicastRemoteObject implements ILoginServerRMI, IXmlReader {
	...
	public boolean isAccountOnServer(String account) {
		for(GameServerInfo gameServerInfo : getGameServers()) {
			try {
				if (gameServerInfo.getConnection() != null && gameServerInfo.getConnection().isAccountOnServer(account)) {
					return true;
				}
			}
			catch (Exception e) {
