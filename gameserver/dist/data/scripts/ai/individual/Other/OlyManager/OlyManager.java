	...
	public String onAdvEvent(String event, Npc npc, Player player) {
		...
		switch (event) {
			...
			case "joinMatch": {
				if (OlympiadManager.getInstance().isRegistered(player)) {
					...
					switch (LocalDate.now().get(WeekFields.of(DayOfWeek.MONDAY, 7).weekOfMonth())) {
						...
						{
							htmltext = getHtm(player.getLang(), "OlyManager-joinMatch.html");
							break;
						}
						default:// Rest is only 1v1 class matches
						{
							htmltext = getHtm(player.getLang(), "OlyManager-joinMatchClass.html");
							break;
						}
					}
			...
			case "rank_189": // Sayha's Seer
			{
				...
				htmltext = getHtm(player.getLang(), "OlyManager-rankDetail.html");
