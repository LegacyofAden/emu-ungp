...
public class BuyShadowItem implements IBypassHandler {
	...
	public boolean useBypass(String command, Player activeChar, Creature target) {
		...
		if (activeChar.getLevel() < 40) {
			html.setFile(activeChar.getLang(), "common/shadow_item-lowlevel.htm");
		} else if ((activeChar.getLevel() >= 40) && (activeChar.getLevel() < 46)) {
			html.setFile(activeChar.getLang(), "common/shadow_item_d.htm");
		} else if ((activeChar.getLevel() >= 46) && (activeChar.getLevel() < 52)) {
			html.setFile(activeChar.getLang(), "common/shadow_item_c.htm");
		} else if (activeChar.getLevel() >= 52) {
			html.setFile(activeChar.getLang(), "common/shadow_item_b.htm");
		}
