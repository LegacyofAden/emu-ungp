...
public final class AdminPForge implements IAdminCommandHandler {
	...
	private void showValuesPage(Player activeChar, String[] opCodes, String format) {
		...
		if (format == null) {
			...
			String editorTemplate = HtmRepository.getInstance().getCustomHtm("admin/pforge/inc/editor.htm");
		...
		if (command.equals("admin_forge")) {
			...
			try {
				...
				if (method.equals("sc")) {
					...
					// @formatter:off
					/*bb.flip();
					L2GameClientPacket p = (L2GameClientPacket) GameServerInfo.gameServer.getL2GamePacketHandler().handlePacket(bb, activeChar.getClient());
					if (p != null)
					{
