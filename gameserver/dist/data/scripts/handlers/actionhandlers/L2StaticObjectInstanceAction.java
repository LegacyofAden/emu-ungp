		...
		if (activeChar.getTarget() != staticObject) {
			...
			if (!activeChar.isInRadius2d(staticObject, Npc.INTERACTION_DISTANCE)) {
				...
				if (staticObject.getType() == 2) {
					final String filename = (staticObject.getId() == 24230101) ? "signboards/tomb_of_crystalgolem.htm" : "signboards/pvp_signboard.htm";
					final String content = HtmRepository.getInstance().getCustomHtm(filename);
