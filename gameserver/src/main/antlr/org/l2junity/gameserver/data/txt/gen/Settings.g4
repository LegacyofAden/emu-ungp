...
minimum_stat
    returns[List<CharacterStatTemplate> value = new ArrayList<>()]:
    'minimum_stat_begin'
...
character_stat
    returns[CharacterStatTemplate value]:
    crc=character_race_class '=' il=int_list
    {$ctx.value = new CharacterStatTemplate($crc.race, $crc.klass, $il.value);}
    ;
...
character_race_class
    ...
    | DWARF_MAGE {$ctx.race = CharacterRace.DWARF; $ctx.klass = CharacterClass.DWARF_MAGE;}
