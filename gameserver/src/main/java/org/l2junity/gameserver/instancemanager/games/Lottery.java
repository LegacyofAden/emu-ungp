...
public class Lottery {
				...
				if (rset.next()) {
					...
					if (rset.getInt("finished") == 1) {
						...
						if (_enddate > System.currentTimeMillis()) {
							_isStarted = true;
							ThreadPool.getInstance().scheduleGeneral(new finishLottery(), _enddate - System.currentTimeMillis(), TimeUnit.MILLISECONDS);

							if (_enddate > (System.currentTimeMillis() + (12 * MINUTE))) {
								_isSellingTickets = true;
								ThreadPool.getInstance().scheduleGeneral(new stopSellingTickets(), _enddate - System.currentTimeMillis() - (10 * MINUTE), TimeUnit.MILLISECONDS);
							}
							return;
						}
					}
	...
	private class finishLottery implements Runnable {
		...
		public void run() {
			...
			ThreadPool.getInstance().scheduleGeneral(new startLottery(), MINUTE, TimeUnit.MILLISECONDS);
			_number++;
