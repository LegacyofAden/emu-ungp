			...
			try (Statement st = con.createStatement();
				...
				while (rs.next()) {
					...
					if (commissionItem.getEndTime().isBefore(Instant.now())) {
						...
						commissionItem.setSaleEndTask(ThreadPool.getInstance().scheduleGeneral(() -> expireSale(commissionItem), Duration.between(Instant.now(), commissionItem.getEndTime()).toMillis(), TimeUnit.MILLISECONDS));
					}
				}
			}
			...
			try (Connection con = DatabaseFactory.getInstance().getConnection();
				...
				try (ResultSet rs = ps.getGeneratedKeys()) {
					if (rs.next()) {
						...
						final ScheduledFuture<?> saleEndTask = ThreadPool.getInstance().scheduleGeneral(() -> expireSale(commissionItem), Duration.between(Instant.now(), commissionItem.getEndTime()).toMillis(), TimeUnit.MILLISECONDS);
