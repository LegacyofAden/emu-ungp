...
public final class FourSepulchersChangeAttackTimeTask implements Runnable {
	...
	public void run() {
		...
		if (manager.isFirstTimeRun()) {
			for (double min = Calendar.getInstance().get(Calendar.MINUTE); min < manager.getCycleMin(); min++) {
				...
				if ((min % 5) == 0)// check if min can be divided by 5
				{
					...
					ThreadPool.getInstance().scheduleGeneral(new FourSepulchersManagerSayTask(), inter.getTimeInMillis() - Calendar.getInstance().getTimeInMillis(), TimeUnit.MILLISECONDS);
					break;
				}
			}
		} else {
			ThreadPool.getInstance().scheduleGeneral(new FourSepulchersManagerSayTask(), 5 * 60400, TimeUnit.MILLISECONDS);
		}
		...
		manager.setChangeCoolDownTimeTask(ThreadPool.getInstance().scheduleGeneral(new FourSepulchersChangeCoolDownTimeTask(), interval, TimeUnit.MILLISECONDS));
