...
public final class GeoData
{
	...
	@Load(group = SideLoadGroup.class)
	private void load()
	{
		int loadedRegions = 0;
		try
			...
			for (int regionX = World.TILE_X_MIN; regionX <= World.TILE_X_MAX; regionX++)
			{
				for (int regionY = World.TILE_Y_MIN; regionY <= World.TILE_Y_MAX; regionY++)
				{
					...
					if (loadFile != null)
					{
						if (loadFile)
						{
							LOGGER.debug("Loading " + geoFilePath.getFileName() + "...");
		...
		catch (Exception e)
		{
			...
			ShutdownManager.halt(TerminationStatus.ENVIRONMENT_MISSING_COMPONENT_OR_SERVICE);
		}
		
		LOGGER.info("Loaded {} geodata region(s).", loadedRegions);
	}
	...
	{
		protected final static GeoData INSTANCE = new GeoData();
	}
}
