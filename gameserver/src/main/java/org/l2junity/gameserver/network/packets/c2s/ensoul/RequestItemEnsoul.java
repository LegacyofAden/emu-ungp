 ...
 */
package org.l2junity.gameserver.network.packets.c2s.ensoul;
...
import org.l2junity.gameserver.network.packets.s2c.ensoul.ExEnsoulResult;
 ...
 * @author UnAfraid
 */
public class RequestItemEnsoul extends GameClientPacket {
	...
	public void readImpl() {
		...
		if ((options > 0) && (options <= 3)) {
			...
			for (int i = 0; i < options; i++) {
				final int slotType = readC();
				final int position = readC();
				final int soulCrystalObjectId = readD();
				final int soulCrystalOption = readD();
