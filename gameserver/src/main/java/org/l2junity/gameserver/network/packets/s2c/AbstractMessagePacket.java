...
public abstract class AbstractMessagePacket<T extends AbstractMessagePacket<?>> extends GameServerPacket {
	...
	protected final void writeMe(PacketBody body) {
		writeParamsSize(body, _params.length);
		SMParam param;
		for (int i = 0; i < _paramIndex; i++) {
			param = _params[i];

			writeParamType(body, param.getType());
			switch (param.getType()) {
				case TYPE_ELEMENT_NAME:
				case TYPE_BYTE: {
					body.writeC(param.getIntValue());
					break;
				}
				...
				case TYPE_NPC_NAME:
				case TYPE_DOOR_NAME: {
					body.writeD(param.getIntValue());
					break;
				}

				case TYPE_LONG_NUMBER: {
					body.writeQ(param.getLongValue());
					break;
				}

				case TYPE_TEXT:
				case TYPE_PLAYER_NAME: {
					body.writeS(param.getStringValue());
					break;
				}

				case TYPE_SKILL_NAME: {
					final int[] array = param.getIntArrayValue();
					body.writeD(array[0]); // skill id
					body.writeH(array[1]); // skill level
					body.writeH(array[2]); // skill sub level
					break;
				}
				...
				case TYPE_ZONE_NAME: {
					final int[] array = param.getIntArrayValue();
					body.writeD(array[0]); // x
					body.writeD(array[1]); // y
					body.writeD(array[2]); // z
					break;
				}
			}
		}
