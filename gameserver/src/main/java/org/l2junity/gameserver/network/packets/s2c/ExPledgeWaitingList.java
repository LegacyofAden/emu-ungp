 ...
 * @author Sdw
 */
public class ExPledgeWaitingList extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.EX_PLEDGE_WAITING_LIST.writeId(body);

		body.writeD(pledgePlayerRecruitInfos.size());
		for (PledgeApplicantInfo recruitInfo : pledgePlayerRecruitInfos.values()) {
			body.writeD(recruitInfo.getPlayerId());
			body.writeS(recruitInfo.getPlayerName());
			body.writeD(recruitInfo.getClassId());
			body.writeD(recruitInfo.getPlayerLvl());
		}
	}
}