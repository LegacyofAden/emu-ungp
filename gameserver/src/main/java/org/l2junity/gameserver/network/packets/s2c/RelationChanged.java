...
public final class RelationChanged extends GameServerPacket {
	...
	public static final int RELATION_DECLARED_WAR = 0x04000; // double fist
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.RELATION_CHANGED.writeId(body);

		body.writeC(_mask);
		if (_multi == null) {
			writeRelation(body, _singled);
		} else {
			body.writeH(_multi.size());
			for (Relation r : _multi) {
				writeRelation(body, r);
			}
		}
	}

	private void writeRelation(PacketBody body, Relation relation) {
		body.writeD(relation._objId);

		if ((_mask & SEND_DEFAULT) == 0) {
			body.writeD(relation._relation);
			body.writeC(relation._autoAttackable);
			body.writeD(relation._reputation);
			body.writeC(relation._pvpFlag);
		}
	}
}
