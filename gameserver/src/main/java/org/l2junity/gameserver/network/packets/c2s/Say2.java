...
public final class Say2 extends GameClientPacket {
	...
	public void runImpl() {
		...
		if (GeneralConfig.LOG_CHAT) {
			if (chatType == ChatType.WHISPER) {
				log.info("{} [{} to {}] {}", chatType.name(), activeChar, _target, _text);
			} else {
				log.info("{} [{}] {}", chatType.name(), activeChar, _text);
			}

		}

		if (_text.indexOf(8) >= 0) {
			if (!parseAndPublishItem(getClient(), activeChar)) {
				return;
			}
		}
	...
	private boolean parseAndPublishItem(GameClient client, Player owner) {
		...
		while ((pos1 = _text.indexOf(8, pos1)) > -1) {
			...
			if (item == null) {
				log.info(client + " trying publish item which doesnt own! ID:" + id);
				return false;
			}
			item.publish();
			...
			{
				log.info(client + " sent invalid publish item msg! ID:" + id);
				return false;
			}
		}
		return true;
