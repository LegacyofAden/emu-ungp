 ...
 * @author Zoey76
 */
public class HennaItemDrawInfo extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.HENNA_ITEM_INFO.writeId(body);

		body.writeD(_henna.getDyeId()); // symbol Id
		body.writeD(_henna.getDyeItemId()); // item id of dye
		body.writeQ(_henna.getWearCount()); // total amount of dye require
		body.writeQ(_henna.getWearFee()); // total amount of Adena require to draw symbol
		body.writeD(_henna.isAllowedClass(_activeChar.getClassId()) ? 0x01 : 0x00); // able to draw or not 0 is false and 1 is true
