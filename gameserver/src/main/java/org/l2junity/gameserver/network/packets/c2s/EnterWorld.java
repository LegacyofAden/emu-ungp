...
import org.l2junity.gameserver.network.packets.s2c.Die;
...
import org.l2junity.gameserver.network.packets.s2c.ExBeautyItemList;
import org.l2junity.gameserver.network.packets.s2c.ExCastleState;
import org.l2junity.gameserver.network.packets.s2c.ExConnectedTimeAndGettableReward;
import org.l2junity.gameserver.network.packets.s2c.ExGetBookMarkInfoPacket;
import org.l2junity.gameserver.network.packets.s2c.ExNoticePostArrived;
import org.l2junity.gameserver.network.packets.s2c.ExNotifyPremiumItem;
import org.l2junity.gameserver.network.packets.s2c.ExPCCafePointInfo;
...
import org.l2junity.gameserver.network.packets.s2c.ExQuestItemList;
...
import org.l2junity.gameserver.network.packets.s2c.ExStorageMaxCount;
...
import org.l2junity.gameserver.network.packets.s2c.ExVitalityEffectInfo;
...
import org.l2junity.gameserver.network.packets.s2c.HennaInfo;
import org.l2junity.gameserver.network.packets.s2c.ItemList;
...
import org.l2junity.gameserver.network.packets.s2c.QuestList;
...
import org.l2junity.gameserver.network.packets.s2c.SkillList;
import org.l2junity.gameserver.network.packets.s2c.SystemMessage;
import org.l2junity.gameserver.network.packets.s2c.ability.ExAcquireAPSkillList;
import org.l2junity.gameserver.network.packets.s2c.friend.L2FriendList;
...
public class EnterWorld extends GameClientPacket {
	...
	public void runImpl() {
		...
		if (activeChar == null) {
			...
			Disconnection.of(getClient()).defaultSequence(false);
			return;
		}
		...
		getClient().sendPacket(new ExGetBookMarkInfoPacket(activeChar));

		// Send Item List
		getClient().sendPacket(new ItemList(activeChar, false));

		// Send Quest Item List
		getClient().sendPacket(new ExQuestItemList(activeChar));

		// Send Adena and Inventory Count
		getClient().sendPacket(new ExAdenaInvenCount(activeChar));

		// Send Shortcuts
		getClient().sendPacket(new ShortCutInit(activeChar));
		...
		// Friend list
		getClient().sendPacket(new L2FriendList(activeChar));
		...
		if (activeChar.isAlikeDead()) // dead or fake dead
		{
			...
			getClient().sendPacket(new Die(activeChar));
		}
		...
		if (GeneralConfig.ALLOW_MAIL) {
			if (MailManager.getInstance().hasUnreadPost(activeChar)) {
				getClient().sendPacket(ExNoticePostArrived.valueOf(false));
			}
		}
