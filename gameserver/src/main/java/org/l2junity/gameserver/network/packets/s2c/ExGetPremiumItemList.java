...
public class ExGetPremiumItemList extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.EX_GET_PREMIUM_ITEM_LIST.writeId(body);

		body.writeD(_map.size());
		for (Entry<Integer, PremiumItem> entry : _map.entrySet()) {
			PremiumItem item = entry.getValue();
			body.writeQ(entry.getKey());
			body.writeD(item.getItemId());
			body.writeQ(item.getCount());
			body.writeD(0x00); // ?
			body.writeS(item.getSender());
		}
	}
}