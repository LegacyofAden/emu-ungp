...
public class MultiSellChoose extends GameClientPacket {
	...
	public void readImpl() {
		...
		_amount = readQ();
		_enchantLevel = readH();
		_augmentOption1 = readD();
		_augmentOption2 = readD();
		_attackAttribute = (short) readH();
		_attributePower = (short) readH();
		_fireDefence = (short) readH();
		_waterDefence = (short) readH();
		_windDefence = (short) readH();
		_earthDefence = (short) readH();
		_holyDefence = (short) readH();
		_darkDefence = (short) readH();
		_soulCrystalOptions = new EnsoulOption[readC()]; // Ensoul size
		for (int i = 0; i < _soulCrystalOptions.length; i++) {
			final int ensoulId = readD(); // Ensoul option id
			_soulCrystalOptions[i] = EnsoulData.getInstance().getOption(ensoulId);
		}
		_soulCrystalSpecialOptions = new EnsoulOption[readC()]; // Special ensoul size
		for (int i = 0; i < _soulCrystalSpecialOptions.length; i++) {
			final int ensoulId = readD(); // Special ensoul option id.
	...
	public void runImpl() {
		...
		if (((_soulCrystalOptions != null) && ArrayUtil.contains(_soulCrystalOptions, null)) || ((_soulCrystalSpecialOptions != null) && ArrayUtil.contains(_soulCrystalSpecialOptions, null))) {
			log.error("Character: {} requested multisell entry with invalid soul crystal options. Multisell: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
		...
		if (entry == null) {
			log.error("Character: {} requested inexistant prepared multisell entry. Multisell: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}

		if (!entry.isStackable() && (_amount > 1)) {
			log.error("Character: {} is trying to set amount > 1 on non-stackable multisell. Id: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
		...
		//@formatter:on
		{
			log.error("Character: {} is trying to upgrade equippable item, but the stats doesn't match. Id: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
			...
			for (ItemHolder ingredient : entry.getIngredients()) {
				...
				if (specialItem != null) {
					...
					switch (specialItem) {
						...
						default: {
							log.error("Character: {} has suffered possible item loss by using multisell {} which has non-implemented special ingredient with id: {}.", player.getName(), _listId, ingredient.getId());
							return;
						}
					}
			...
			for (ItemChanceHolder product : products) {
				...
				if (specialItem != null) {
					...
					switch (specialItem) {
						...
						default: {
							log.error("Character: {} has suffered possible item loss by using multisell {} which has non-implemented special product with id: {}.", player.getName(), _listId, product.getId());
							return;
						}
					}
				} else {
	...
	private boolean checkIngredients(final Player player, PreparedMultisellListHolder list, final PcInventory inventory, final Clan clan, final int ingredientId, final long totalCount) {
		...
		if (specialItem != null) {
			...
			switch (specialItem) {
				...
				default: {
					log.error("Multisell: {} is using a non-implemented special ingredient with id: {}.", _listId, ingredientId);
					return false;
				}
			}
		}
