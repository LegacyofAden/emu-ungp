...
public class SellListProcure extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.SELL_LIST_PROCURE.writeId(body);

		body.writeQ(_money); // money
		body.writeD(0x00); // lease ?
		body.writeH(_sellList.size()); // list size

		for (ItemInstance item : _sellList.keySet()) {
			body.writeH(item.getItem().getType1());
			body.writeD(item.getObjectId());
			body.writeD(item.getDisplayId());
			body.writeQ(_sellList.get(item)); // count
			body.writeH(item.getItem().getType2());
			body.writeH(0); // unknown
			body.writeQ(0); // price, u shouldnt get any adena for crops, only raw materials
		}
	}
}
