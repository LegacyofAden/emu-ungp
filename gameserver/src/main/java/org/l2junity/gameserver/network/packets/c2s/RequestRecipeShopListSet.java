...
import org.l2junity.gameserver.network.packets.s2c.RecipeShopMsg;
...
public final class RequestRecipeShopListSet extends GameClientPacket {
	...
	public void runImpl() {
		...
		if (AttackStanceTaskManager.getInstance().hasAttackStanceTask(player) || player.isInDuel()) {
			getClient().sendPacket(SystemMessageId.WHILE_YOU_ARE_ENGAGED_IN_COMBAT_YOU_CANNOT_OPERATE_A_PRIVATE_STORE_OR_PRIVATE_WORKSHOP);
			getClient().sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}

		if (player.isInsideZone(ZoneId.NO_STORE)) {
			getClient().sendPacket(SystemMessageId.YOU_CANNOT_OPEN_A_PRIVATE_WORKSHOP_HERE);
			getClient().sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
