...
public class Die extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.DIE.writeId(body);

		body.writeD(_objectId);
		body.writeD(_toVillage ? 0x01 : 0x00);
		body.writeD(_toClanHall ? 0x01 : 0x00);
		body.writeD(_toCastle ? 0x01 : 0x00);
		body.writeD(_toOutpost ? 0x01 : 0x00);
		body.writeD(_isSweepable ? 0x01 : 0x00);
		body.writeD(_useFeather ? 0x01 : 0x00);
		body.writeD(_toFortress ? 0x01 : 0x00);
		body.writeD(0x00); // Disables use Feather button for X seconds
		body.writeD(0x00); // Adventure's Song
		body.writeC(_hideAnimation ? 0x01 : 0x00);

		body.writeD(_itemsEnabled ? 0x01 : 0x00);
		body.writeD(getItems().size());
		getItems().forEach(body::writeD);
	}
}
