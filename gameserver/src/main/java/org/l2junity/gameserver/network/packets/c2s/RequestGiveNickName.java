...
public class RequestGiveNickName extends GameClientPacket {
	...
	public void runImpl() {
		...
		if (activeChar.isNoble() && _target.equalsIgnoreCase(activeChar.getName())) {
			...
			getClient().sendPacket(SystemMessageId.YOUR_TITLE_HAS_BEEN_CHANGED);
			...
			if (activeChar.getClan().getLevel() < 3) {
				getClient().sendPacket(SystemMessageId.A_PLAYER_CAN_ONLY_BE_GRANTED_A_TITLE_IF_THE_CLAN_IS_LEVEL_3_OR_ABOVE);
				return;
			}
			...
			if (member1 != null) {
				...
				if (member != null) {
					...
					getClient().sendPacket(SystemMessageId.THAT_PLAYER_IS_NOT_ONLINE);
				}
			} else {
				getClient().sendPacket(SystemMessageId.THE_TARGET_MUST_BE_A_CLAN_MEMBER);
			}
		}
	}
}
