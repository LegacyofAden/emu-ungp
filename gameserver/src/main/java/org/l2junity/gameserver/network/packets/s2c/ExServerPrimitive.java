...
public class ExServerPrimitive extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.EX_SERVER_PRIMITIVE.writeId(body);

		body.writeS(_name);
		body.writeD(_x);
		body.writeD(_y);
		body.writeD(_z);
		body.writeD(65535); // has to do something with display range and angle
		body.writeD(65535); // has to do something with display range and angle

		body.writeD(_points.size() + _lines.size());

		for (Point point : _points) {
			body.writeC(1); // Its the type in this case Point
			body.writeS(point.getName());
			int color = point.getColor();
			body.writeD((color >> 16) & 0xFF); // R
			body.writeD((color >> 8) & 0xFF); // G
			body.writeD(color & 0xFF); // B
			body.writeD(point.isNameColored() ? 1 : 0);
			body.writeD(point.getX());
			body.writeD(point.getY());
			body.writeD(point.getZ());
		}

		for (Line line : _lines) {
			body.writeC(2); // Its the type in this case Line
			body.writeS(line.getName());
			int color = line.getColor();
			body.writeD((color >> 16) & 0xFF); // R
			body.writeD((color >> 8) & 0xFF); // G
			body.writeD(color & 0xFF); // B
			body.writeD(line.isNameColored() ? 1 : 0);
			body.writeD(line.getX());
			body.writeD(line.getY());
			body.writeD(line.getZ());
			body.writeD(line.getX2());
			body.writeD(line.getY2());
			body.writeD(line.getZ2());
		}
	}
