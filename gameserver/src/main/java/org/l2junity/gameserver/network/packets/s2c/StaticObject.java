...
public class StaticObject extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.STATIC_OBJECT.writeId(body);

		body.writeD(_staticObjectId);
		body.writeD(_objectId);
		body.writeD(_type);
		body.writeD(_isTargetable ? 1 : 0);
		body.writeD(_meshIndex);
		body.writeD(_isClosed ? 1 : 0);
		body.writeD(_isEnemy ? 1 : 0);
		body.writeD(_currentHp);
		body.writeD(_maxHp);
		body.writeD(_showHp ? 1 : 0);
		body.writeD(_damageGrade);
	}
}