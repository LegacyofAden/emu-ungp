...
public final class HennaInfo extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.HENNA_INFO.writeId(body);
		...
		body.writeD(3 - _activeChar.getHennaEmptySlots()); // Slots
		body.writeD(_hennas.size()); // Size
		for (Henna henna : _hennas) {
			body.writeD(henna.getDyeId());
			body.writeD(henna.isAllowedClass(_activeChar.getClassId()) ? 0x01 : 0x00);
		}
		body.writeD(0x00); // Premium Slot Dye ID
		body.writeD(0x00); // Premium Slot Dye Time Left
		body.writeD(0x00); // Premium Slot Dye ID isValid
	}
}
