...
import org.l2junity.gameserver.network.packets.s2c.ExEnchantSkillResult;
...
public final class RequestExEnchantSkill extends GameClientPacket {
	...
	public void readImpl() {
		...
		if ((type < 0) || (type >= SkillEnchantType.values().length)) {
			LOGGER.warn("Client: {} send incorrect type {} on packet: {}", getClient(), type, getClass().getSimpleName());
			return;
		}
	...
	public void runImpl() {
		...
		if (skill.getSubLevel() > 0) {
			if (_type == SkillEnchantType.CHANGE) {
				...
				if (group1 != group2) {
					LOGGER.warn("{}: Client: {} send incorrect sub level group: {} expected: {}", getClass().getSimpleName(), getClient(), group1, group2);
					return;
				}
			} else if ((skill.getSubLevel() + 1) != _skillSubLvl) {
				LOGGER.warn("{}: Client: {} send incorrect sub level: {} expected: {}", getClass().getSimpleName(), getClient(), _skillSubLvl, skill.getSubLevel() + 1);
				return;
			}
		}
