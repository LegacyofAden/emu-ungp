...
public class ExSendUIEvent extends GameServerPacket {
	...
	protected void writeImpl(PacketBody body) {
		GameServerPacketType.EX_SEND_UIEVENT.writeId(body);

		body.writeD(_objectId);
		body.writeD(_type); // 0 = show, 1 = hide (there is 2 = pause and 3 = resume also but they don't work well you can only pause count down and you cannot resume it because resume hides the counter).
		body.writeD(0);// unknown
		body.writeD(0);// unknown
		body.writeS(String.valueOf(_countUp)); // 0 = count down, 1 = count up timer always disappears 10 seconds before end
		body.writeS(String.valueOf(_startTime));
		body.writeS(String.valueOf(_startTime2));
		body.writeS(String.valueOf(_endTime));
		body.writeS(String.valueOf(_endTime2));
		body.writeD(_npcstringId);
		if (_params != null) {
