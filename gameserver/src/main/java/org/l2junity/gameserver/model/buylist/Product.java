...
public final class Product {
	...
	public boolean decreaseCount(long val) {
		...
		if ((_restockTask == null) || _restockTask.isDone()) {
			_restockTask = ThreadPool.getInstance().scheduleGeneral(this::restock, getRestockDelay(), TimeUnit.MILLISECONDS);
		}
	...
	public void restartRestockTask(long nextRestockTime) {
		...
		if (remainTime > 0) {
			_restockTask = ThreadPool.getInstance().scheduleGeneral(this::restock, remainTime, TimeUnit.MILLISECONDS);
		} else {
			restock();
		}
	}
	...
	private void save() {
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			...
			_log.warn("Failed to save Product buylist_id:" + _buyListId + " item_id:" + getItemId(), e);
		}
	}
}
