...
public final class Formulas {
	...
	public static boolean calcCrit(double rate, Creature activeChar, Creature target, Skill skill) {
		...
		if (skill != null) {
			...
			byte skillCritRateStat = (byte) activeChar.getStat().getValue(DoubleStat.STAT_BONUS_SKILL_CRITICAL, -1);
			if ((skillCritRateStat >= 0) && (skillCritRateStat < BaseStats.values().length)) {
				// Best tested
				statBonus = BaseStats.STR.getValue(activeChar.getDEX()) * 2;
				rateBonus = (activeChar.getStat().getValue(DoubleStat.CRITICAL_RATE_SKILL) * 2) - 1; // Tests made by retail GMs show that 3x10% increase yields to 16.2 -> 26.1
			} else {
				// Default base stat used for skill critical formula is STR.
				statBonus = BaseStats.STR.calcBonus(activeChar);
				rateBonus = activeChar.getStat().getValue(DoubleStat.CRITICAL_RATE_SKILL);
			}
		...
		rate *= getCritPositionBonus(activeChar, target);
		...
		double finalRate = target.getStat().getValue(DoubleStat.DEFENCE_CRITICAL_RATE, rate) + target.getStat().getValue(DoubleStat.DEFENCE_CRITICAL_RATE_ADD, 0);
		return finalRate > Rnd.get(1000);
	}
	 ...
	 */
	public static double getCritPositionBonus(Creature activeChar, Creature target) {
	...
	public static boolean calcSkillMastery(Creature actor, Skill sk) {
		...
		final int val = (int) actor.getStat().getValue(DoubleStat.SKILL_CRITICAL, -1);
	...
	public static boolean calcBlowSuccess(Creature activeChar, Creature target, Skill skill, double blowChance) {
		...
		final double criticalPosition = getCritPositionBonus(activeChar, target); // 30% chance from back, 10% chance from side. Include buffs that give positional crit rate.
		final double blowRateMod = activeChar.getStat().getValue(DoubleStat.BLOW_RATE, 1);
		blowChance = 1 + (blowChance / 10);

		final double rate = weaponCritical * blowChance * critHeightBonus * criticalPosition * blowRateMod;
