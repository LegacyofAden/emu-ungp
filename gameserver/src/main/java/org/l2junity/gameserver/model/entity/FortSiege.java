...
public class FortSiege implements Siegable {
	...
	public class ScheduleStartSiegeTask implements Runnable {
		...
		public void run() {
			...
			try {
				...
				{
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(600), 3000000, TimeUnit.MILLISECONDS); // Prepare task for 10 minutes left.
				} else if (_time == 600) // 10min remains
				{
					...
					sm.addInt(10);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(300), 300000, TimeUnit.MILLISECONDS); // Prepare task for 5 minutes left.
				} else if (_time == 300) // 5min remains
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.S1_MINUTE_S_UNTIL_THE_FORTRESS_BATTLE_STARTS);
					sm.addInt(5);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(60), 240000, TimeUnit.MILLISECONDS); // Prepare task for 1 minute left.
				} else if (_time == 60) // 1min remains
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.S1_MINUTE_S_UNTIL_THE_FORTRESS_BATTLE_STARTS);
					sm.addInt(1);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(30), 30000, TimeUnit.MILLISECONDS); // Prepare task for 30 seconds left.
				} else if (_time == 30) // 30seconds remains
				{
					...
					sm.addInt(30);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(10), 20000, TimeUnit.MILLISECONDS); // Prepare task for 10 seconds left.
				} else if (_time == 10) // 10seconds remains
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.S1_SECOND_S_UNTIL_THE_FORTRESS_BATTLE_STARTS);
					sm.addInt(10);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(5), 5000, TimeUnit.MILLISECONDS); // Prepare task for 5 seconds left.
				} else if (_time == 5) // 5seconds remains
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.S1_SECOND_S_UNTIL_THE_FORTRESS_BATTLE_STARTS);
					sm.addInt(5);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(1), 4000, TimeUnit.MILLISECONDS); // Prepare task for 1 seconds left.
				} else if (_time == 1) // 1seconds remains
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.S1_SECOND_S_UNTIL_THE_FORTRESS_BATTLE_STARTS);
					sm.addInt(1);
					announceToPlayer(sm);
					ThreadPool.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(0), 1000, TimeUnit.MILLISECONDS); // Prepare task start siege.
	...
	public void endSiege() {
		if (isInProgress()) {
			...
			ThreadPool.getInstance().scheduleGeneral(new ScheduleSuspiciousMerchantSpawn(), FortSiegeManager.getInstance().getSuspiciousMerchantRespawnDelay() * 60 * 1000L, TimeUnit.MILLISECONDS); // Prepare 3hr task for suspicious merchant respawn
	...
	public void startSiege() {
		if (!isInProgress()) {
			...
			_siegeEnd = ThreadPool.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(), FortSiegeManager.getInstance().getSiegeLength() * 60 * 1000L, TimeUnit.MILLISECONDS); // Prepare auto end task
		...
		if ((getFort() != null) && (!_commanders.isEmpty())) {
			...
			if (spawn != null) {
				...
				// schedule restoring doors/commanders respawn
					...
					_siegeRestore = ThreadPool.getInstance().scheduleGeneral(new ScheduleSiegeRestore(), FortSiegeManager.getInstance().getCountDownLength() * 60 * 1000L, TimeUnit.MILLISECONDS);
				} else {
					getFort().getSiege().announceToPlayer(SystemMessage.getSystemMessage(SystemMessageId.THE_BARRACKS_HAVE_BEEN_SEIZED));
				}
			} else {
	...
	public void checkAutoTask() {
		...
		if (delay < 0) {
			...
			ThreadPool.getInstance().executeGeneral(new ScheduleSuspiciousMerchantSpawn());
		} else {
			loadSiegeClan();
			if (getAttackerClans().isEmpty()) {
				// no attackers - waiting for suspicious merchant spawn
				ThreadPool.getInstance().scheduleGeneral(new ScheduleSuspiciousMerchantSpawn(), delay, TimeUnit.MILLISECONDS);
			} else {
				// preparing start siege task
				if (delay > 3600000) // more than hour, how this can happens ? spawn suspicious merchant
				{
					ThreadPool.getInstance().executeGeneral(new ScheduleSuspiciousMerchantSpawn());
					_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(3600), delay - 3600000, TimeUnit.MILLISECONDS);
				}
				if (delay > 600000) // more than 10 min, spawn suspicious merchant
				{
					ThreadPool.getInstance().executeGeneral(new ScheduleSuspiciousMerchantSpawn());
					_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(600), delay - 600000, TimeUnit.MILLISECONDS);
				} else if (delay > 300000) {
					_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(300), delay - 300000, TimeUnit.MILLISECONDS);
				} else if (delay > 60000) {
					_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(60), delay - 60000, TimeUnit.MILLISECONDS);
				} else {
					// lower than 1 min, set to 1 min
					_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(60), 0, TimeUnit.MILLISECONDS);
				}
	...
	public void startAutoTask(boolean setTime) {
		...
		_siegeStartTask = ThreadPool.getInstance().scheduleGeneral(new FortSiege.ScheduleStartSiegeTask(3600), 0, TimeUnit.MILLISECONDS);
	}
