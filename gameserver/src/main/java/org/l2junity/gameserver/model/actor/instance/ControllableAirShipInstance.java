...
public class ControllableAirShipInstance extends AirShipInstance {
	...
	public void onSpawn() {
		super.onSpawn();
		_checkTask = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new CheckTask(), 60000, 10000, TimeUnit.MILLISECONDS);
		_consumeFuelTask = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new ConsumeFuelTask(), 60000, 60000, TimeUnit.MILLISECONDS);
	}
	...
	protected final class CheckTask implements Runnable {
		...
		public void run() {
			if (isSpawned() && isEmpty() && !isInDock()) {
				...
				ThreadPool.getInstance().executeGeneral(new DecayTask());
			}
		}
	}
