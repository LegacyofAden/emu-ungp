...
public class L2Spawn extends Location implements IIdentifiable, INamable {
	...
	public void decreaseCount(Npc oldNpc) {
		...
		if (_doRespawn && ((_scheduledCount + _currentCount) < _maximumCount)) {
			...
			ThreadPool.getInstance().scheduleGeneral(new SpawnTask(oldNpc), hasRespawnRandom() ? Rnd.get(_respawnMinDelay, _respawnMaxDelay) : _respawnMinDelay, TimeUnit.MILLISECONDS);
		}
	}
