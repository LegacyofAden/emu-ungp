...
import org.l2junity.gameserver.network.packets.s2c.SetSummonRemainTime;
...
public class ServitorInstance extends Summon implements Runnable {
	private static final Logger LOGGER = LoggerFactory.getLogger(ServitorInstance.class);
	...
	public ServitorInstance(NpcTemplate template, Player owner) {
	...
	public void onSpawn() {
		...
		if ((_lifeTime > 0) && (_summonLifeTask == null)) {
			_summonLifeTask = ThreadPool.getInstance().scheduleGeneralAtFixedRate(this, 0, 5000, TimeUnit.MILLISECONDS);
		}
	}
