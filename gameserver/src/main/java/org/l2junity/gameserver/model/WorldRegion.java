...
public final class WorldRegion {
	...
	private void startActivation() {
		...
		synchronized (this) {
			...
			_neighborsTask = ThreadPool.getInstance().scheduleGeneral(new NeighborsTask(true), 1000 * GeneralConfig.GRID_NEIGHBOR_TURNON_TIME, TimeUnit.MILLISECONDS);
		}
	}
	...
	private void startDeactivation() {
		synchronized (this) {
			...
			_neighborsTask = ThreadPool.getInstance().scheduleGeneral(new NeighborsTask(false), 1000 * GeneralConfig.GRID_NEIGHBOR_TURNOFF_TIME, TimeUnit.MILLISECONDS);
		}
	}
