...
public class Duel {
	...
	public Duel(PlayerInstance playerA, PlayerInstance playerB, int partyDuel, int duelId) {
		...
		// Schedule duel start
		ThreadPool.getInstance().scheduleGeneral(new ScheduleStartDuelTask(this), 3000, TimeUnit.MILLISECONDS);
	}
			...
			try {
				switch (_duel.checkEndDuelCondition()) {
					...
					default: {
						setFinished(true);
						playKneelAnimation();
						ThreadPool.getInstance().scheduleGeneral(new ScheduleEndDuelTask(_duel, _duel.checkEndDuelCondition()), 5000, TimeUnit.MILLISECONDS);
	...
	public void startDuel() {
		...
		// start duelling task
		ThreadPool.getInstance().scheduleGeneral(new ScheduleDuelTask(this), 1000, TimeUnit.MILLISECONDS);
	}
