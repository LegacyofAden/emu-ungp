...
public final class BlockCheckerEngine {
	...
	public void endEventAbnormally() {
		try {
			synchronized (this) {
				...
				ThreadPool.getInstance().executeGeneral(new EndEvent());
			}
		} catch (Exception e) {
	...
	public class StartEvent implements Runnable {
		...
		public void run() {
			...
			_isStarted = true;
			// Spawn the blocks
			ThreadPool.getInstance().executeGeneral(new SpawnRound(16, 1));
		...
		public void run() {
			...
			switch (_round) {
				case 1:
					...
					_task = ThreadPool.getInstance().scheduleGeneral(new SpawnRound(20, 2), 60000, TimeUnit.MILLISECONDS);
					break;
				case 2:
					// Schedule third spawn round
					_task = ThreadPool.getInstance().scheduleGeneral(new SpawnRound(14, 3), 60000, TimeUnit.MILLISECONDS);
					break;
				case 3:
					// Schedule Event End Count Down
					_task = ThreadPool.getInstance().scheduleGeneral(new EndEvent(), 180000, TimeUnit.MILLISECONDS);
					break;
			}
			...
			if ((_round == 1) || (_round == 2)) {
				try {
					...
					ThreadPool.getInstance().scheduleGeneral(new CarryingGirlUnspawn(girlSpawn), 9000, TimeUnit.MILLISECONDS);
				} catch (Exception e) {
