		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			...
			if (ownerId > 0) {
				...
				if ((FeatureConfig.FS_MAX_OWN_TIME <= 0) || (getOwnedTime() < (FeatureConfig.FS_MAX_OWN_TIME * 3600))) {
					_FortUpdater[0] = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new FortUpdater(this, clan, runCount, UpdaterType.PERIODIC_UPDATE), initial, FeatureConfig.FS_UPDATE_FRQ * 60000L, TimeUnit.MILLISECONDS); // Schedule owner tasks to start running
					if (FeatureConfig.FS_MAX_OWN_TIME > 0) {
						_FortUpdater[1] = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new FortUpdater(this, clan, runCount, UpdaterType.MAX_OWN_TIME), 3600000, 3600000, TimeUnit.MILLISECONDS); // Schedule owner tasks to remove owener
					}
				} else {
					_FortUpdater[1] = ThreadPool.getInstance().scheduleGeneral(new FortUpdater(this, clan, 0, UpdaterType.MAX_OWN_TIME), 60000, TimeUnit.MILLISECONDS); // Schedule owner tasks to remove owner
				}
			} else {
				setOwnerClan(null);
			}
	...
	private void updateOwnerInDB() {
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
			...
			if (clan != null) {
				...
				_FortUpdater[0] = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new FortUpdater(this, clan, 0, UpdaterType.PERIODIC_UPDATE), FeatureConfig.FS_UPDATE_FRQ * 60000L, FeatureConfig.FS_UPDATE_FRQ * 60000L, TimeUnit.MILLISECONDS); // Schedule owner tasks to start running
				if (FeatureConfig.FS_MAX_OWN_TIME > 0) {
					_FortUpdater[1] = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new FortUpdater(this, clan, 0, UpdaterType.MAX_OWN_TIME), 3600000, 3600000, TimeUnit.MILLISECONDS); // Schedule owner tasks to remove owener
				}
			} else {
				if (_FortUpdater[0] != null) {
					_FortUpdater[0].cancel(false);
