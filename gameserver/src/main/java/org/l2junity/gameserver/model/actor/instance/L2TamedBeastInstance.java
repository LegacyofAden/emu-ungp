...
public final class L2TamedBeastInstance extends L2FeedableBeastInstance {
	...
	public void setFoodType(int foodItemId) {
		if (foodItemId > 0) {
			...
			_durationCheckTask = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new CheckDuration(this), DURATION_CHECK_INTERVAL, DURATION_CHECK_INTERVAL, TimeUnit.MILLISECONDS);
		}
	}
	...
	public void castBeastSkills() {
		...
		for (Skill skill : _beastSkills) {
			ThreadPool.getInstance().scheduleEffect(new buffCast(skill), delay, TimeUnit.MILLISECONDS);
			delay += (100 + skill.getHitTime());
		}
		ThreadPool.getInstance().scheduleEffect(new buffCast(null), delay, TimeUnit.MILLISECONDS);
	}
	...
	public void setOwner(Player owner) {
		if (owner != null) {
			...
			if (!_isFreyaBeast) {
				...
				_buffTask = ThreadPool.getInstance().scheduleGeneralAtFixedRate(new CheckOwnerBuffs(this, totalBuffsAvailable), BUFF_INTERVAL, BUFF_INTERVAL, TimeUnit.MILLISECONDS);
			}
		} else {
