...
import org.l2junity.gameserver.model.eventengine.drop.*;
 ...
 */
@Slf4j
@StartupComponent(value = "Data", dependency = ClanHallData.class)
public final class EventEngineData implements IGameXmlReader {
	...
	private void parseEvent(Node eventNode) {
		...
		try {
			...
			log.warn("Couldn't locate event manager instance for event: {} !", eventName, e);
			return;
		}
		...
		log.info("{}: Initialized", eventManager.getClass().getSimpleName());
	}
				...
				for (Node eventNode = scheduleNode.getFirstChild(); eventNode != null; eventNode = eventNode.getNextSibling()) {
					if ("event".equals(eventNode.getNodeName())) {
						...
						try {
							...
							log.warn("Couldn't add event notification for {}", eventManager.getClass().getSimpleName(), e);
						}
					}
				}
					...
					if ("run".equals(eventNode.getNodeName())) {
						...
						switch (ifType) {
							case "BETWEEN": {
								...
								if (names.size() != 2) {
									log.warn("Event: {} has incorrect amount of scheduler names: {} expected: {} found: {}", eventManager.getClass().getSimpleName(), names, 2, names.size());
								} else {
	...
	private void parseListVariables(AbstractEventManager<?> eventManager, StatsSet variables, Node variableNode) {
		...
		switch (type) {
			...
			default: {
				log.info("Unhandled list case: {} for event: {}", type, eventManager.getClass().getSimpleName());
				break;
			}
		}
	...
	private void parseMapVariables(AbstractEventManager<?> eventManager, StatsSet variables, Node variableNode) {
		...
		forEach(variableNode, IXmlReader::isNode, stringNode ->
		{
			switch (stringNode.getNodeName()) {
				...
				default: {
					log.warn("Unhandled map case: {} {} for event: {}", name, stringNode.getNodeName(), eventManager.getClass().getSimpleName());
				}
			}
		});
	...
	private Class<?> getClassByName(AbstractEventManager<?> eventManager, String name) {
		switch (name) {
			...
			default: {
				log.warn("Unhandled class case: {} for event: {}", name, eventManager.getClass().getSimpleName());
				return Object.class;
			}
		}
	}
	...
	private Object parseObject(AbstractEventManager<?> eventManager, String type, String value) {
		switch (type) {
			...
			default: {
				log.warn("Unhandled object case: {} for event: {}", type, eventManager.getClass().getSimpleName());
				return null;
			}
		}
	}
