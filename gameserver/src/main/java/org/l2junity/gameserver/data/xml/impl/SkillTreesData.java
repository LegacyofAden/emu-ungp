 ...
 */
@Slf4j
@StartupComponent(value = "Data", dependency = SkillData.class)
public final class SkillTreesData implements IGameXmlReader {
	@Getter(lazy = true)
	private static final SkillTreesData instance = new SkillTreesData();
						...
						for (Node c = d.getFirstChild(); c != null; c = c.getNextSibling()) {
							if ("skill".equalsIgnoreCase(c.getNodeName())) {
								...
								switch (type) {
									...
									default: {
										log.warn("Unknown Skill Tree type: " + type + "!");
									}
								}
							}
						}
	...
	private List<SkillLearn> getAvailableSkills(PlayerInstance player, ClassId classId, boolean includeByFs, boolean includeAutoGet, ISkillsHolder holder) {
		...
		final boolean isAwaken = player.isAwakenedClass() && (player.getRace() != Race.ERTHEIA);
		...
		for (Entry<Long, SkillLearn> entry : skills.entrySet()) {
			...
			if (player.getLevel() >= skill.getGetLevel()) {
				if (skill.getSkillLevel() > SkillData.getInstance().getMaxLevel(skill.getSkillId())) {
					log.error("SkillTreesData found learnable skill {} with level higher than max skill level!", skill.getSkillId());
					continue;
				}
	...
	public List<SkillLearn> getAvailableAutoGetSkills(PlayerInstance player) {
		...
		if (skills.isEmpty()) {
			...
			log.warn("Skill Tree for this class Id(" + player.getClassId() + ") is not defined!");
			return result;
		}
	...
	public int getMinLevelForNewSkill(PlayerInstance player, Map<Long, SkillLearn> skillTree) {
		...
		if (skillTree.isEmpty()) {
			log.warn("SkillTree is not defined for getMinLevelForNewSkill!");
		} else {
	...
	private void report() {
		...
		log.info("Loaded {} Dual-Class Skills.", _dualClassSkillTree.size());
		log.info("Loaded {} Transfer Skills for {} Transfer Skill Trees.", transferSkillTreeCount, _transferSkillTrees.size());
		log.info("Loaded {} Race skills for {} Race Skill Trees.", raceSkillTreeCount, _raceSkillTree.size());
		...
		log.info("Loaded {} Ability Skills.", _abilitySkillTree.size());
		log.info("Loaded {} Alchemy Skills.", _alchemySkillTree.size());
		log.info("Loaded {} Class Awaken Save Skills.", _awakeningSaveSkillTree.size());
		log.info("Loaded {} Revelation Skills.", revelationSkillTreeCount);
