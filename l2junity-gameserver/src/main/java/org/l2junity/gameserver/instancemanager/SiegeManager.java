	...
	private void load()
	{
		...
		for (Castle castle : CastleManager.getInstance().getCastles())
		{
			...
			for (int i = 1; i < 0xFF; i++)
			{
				...
				{
					LOGGER.warn(getClass().getSimpleName() + ": Error while loading control tower(s) for " + castle.getName() + " castle.");
				}
			}
			...
			for (int i = 1; i < 0xFF; i++)
			{
				...
				{
					LOGGER.warn(getClass().getSimpleName() + ": Error while loading flame tower(s) for " + castle.getName() + " castle.");
				}
			}
