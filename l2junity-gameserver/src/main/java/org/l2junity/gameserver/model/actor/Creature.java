...
public abstract class Creature extends WorldObject implements ISkillsHolder, IDeletable
{
	...
	public void doAutoAttack(Creature target)
	{
		synchronized (_attackLock)
		{
			...
			ThreadPool.schedule(new NotifyAITask(this, CtrlEvent.EVT_READY_TO_ACT), timeAtk, TimeUnit.MILLISECONDS);
		}
	...
	private boolean doAttackHitByBow(Attack attack, Creature target, int sAtk, int reuse, boolean crossbow)
	{
		...
		ThreadPool.schedule(new HitTask(this, target, damage1, crit1, miss1, attack.hasSoulshot(), shld1), sAtk, TimeUnit.MILLISECONDS);
	...
	private boolean doAttackHitByDual(Attack attack, Creature target, int sAtk, int sAtk2)
	{
		...
		ThreadPool.schedule(new HitTask(this, target, damage2, crit2, miss2, attack.hasSoulshot(), shld2), sAtk2, TimeUnit.MILLISECONDS);
		...
		if (attackCountMax > 1)
		{
			...
			for (Creature surroundTarget : attackSurround)
			{
				...
				ThreadPool.schedule(new HitTask(this, surroundTarget, damage, crit, miss, attack.hasSoulshot(), shld), sAtk, TimeUnit.MILLISECONDS);
			...
			for (Creature surroundTarget : attackSurround)
			{
				...
				ThreadPool.schedule(new HitTask(this, surroundTarget, damage, crit, miss, attack.hasSoulshot(), shld), sAtk2, TimeUnit.MILLISECONDS);
				attack.addHit(surroundTarget, damage, miss, crit, shld);
				miss2 |= miss;
			}
