...
public class MultiSellChoose implements IClientIncomingPacket
{
	...
	public void run(L2GameClient client)
	{
		...
		{
			_log.error("Character: {} requested multisell entry with invalid soul crystal options. Multisell: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
		...
		if (entry == null)
		{
			_log.error("Character: {} requested inexistant prepared multisell entry. Multisell: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
		...
		{
			_log.error("Character: {} is trying to set amount > 1 on non-stackable multisell. Id: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
		...
		//@formatter:on
		{
			_log.error("Character: {} is trying to upgrade equippable item, but the stats doesn't match. Id: {} entry: {}", player.getName(), _listId, _entryId);
			player.setMultiSell(null);
			return;
		}
			...
			for (ItemHolder ingredient : entry.getIngredients())
			{
				...
				if (specialItem != null)
				{
					...
					switch (specialItem)
					{
						...
						default:
						{
							_log.error("Character: {} has suffered possible item loss by using multisell {} which has non-implemented special ingredient with id: {}.", player.getName(), _listId, ingredient.getId());
							return;
						}
					}
				}
			...
			for (ItemChanceHolder product : products)
			{
				...
				if (specialItem != null)
				{
					...
					switch (specialItem)
					{
						...
						default:
						{
							_log.error("Character: {} has suffered possible item loss by using multisell {} which has non-implemented special product with id: {}.", player.getName(), _listId, product.getId());
							return;
						}
					}
				}
	...
	private boolean checkIngredients(final PlayerInstance player, PreparedMultisellListHolder list, final PcInventory inventory, final L2Clan clan, final int ingredientId, final long totalCount)
	{
		...
		if (specialItem != null)
		{
			...
			switch (specialItem)
			{
				...
				default:
				{
					_log.error("Multisell: {} is using a non-implemented special ingredient with id: {}.", _listId, ingredientId);
					return false;
				}
			}
		}
