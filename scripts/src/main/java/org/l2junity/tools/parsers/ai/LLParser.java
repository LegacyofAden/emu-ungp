...
import org.l2junity.gameserver.retail.model.TelPosInfo;
import org.l2junity.gameserver.retail.model.WayPointDelaysType;
import org.l2junity.gameserver.retail.model.WayPointsType;
...
import org.l2junity.tools.parsers.ai.class_model.DBClassInfoContainer;
import org.l2junity.tools.parsers.ai.ll.AiClass;
import org.l2junity.tools.parsers.ai.ll.EventHandler;
import org.l2junity.tools.parsers.ai.ll.ParameterDefine;
import org.l2junity.tools.parsers.ai.ll.PropertyDefine;
import org.l2junity.tools.parsers.ai.ll.stack.*;
import org.l2junity.tools.parsers.ai.ll.stack.EventObject;
...
public class LLParser {
	@Getter(lazy = true)
	private static final LLParser instance = new LLParser();

	private final Set<Integer> notImplementedEvents = new HashSet<>();

	private final Map<String, StringObject> strings = new ConcurrentHashMap<>();

	public void processClass(PlainClassData classData) {
		log.info("Processing class: {}", classData.getClassName());
		AiClass ai_class = new AiClass(classData);

		try (BufferedReader classReader = new BufferedReader(new StringReader(classData.getClassBody()))) {
			...
			DBClassInfoContainer.getInstance().add(classInfo);
		} catch (IOException e) {
			log.error("Error while parsing class [{}]", classData.getClassName(), e);
		}
	}
	...
	private void parseClass(AiClass aiClass, BufferedReader br) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			if (line.startsWith("parameter_define_begin")) {
				TreeSet<ParameterDefine> parameterDefineList = readParameterDefines(line, br, aiClass);
		...
		while (!(line = br.readLine()).equals("property_define_end")) {
			...
			if (line.startsWith("buyselllist_begin")) {
				...
				while (!(line = br.readLine()).equals("telposlist_end")) {
					...
					// 1010619; 104426; 33746; -3825; 2000; {4; 5}
					if (parts.length == 7) {
						// TODO: Last variable can be array ({4; 5}) or just number
						list.add(new TelPosInfo(Integer.parseInt(parts[0].replaceAll("\"", "")), Integer.parseInt(parts[1]),
								Integer.parseInt(parts[2]), Integer.parseInt(parts[3]), Integer.parseInt(parts[4]), 0)); // TODO castleId's
			...
			String parentClassName = aiClass.getParent();
			boolean found = false;

			// Ищем проперти в родительских классах
			// Останавливаем поиск на классах MakerEventHandler и NpcEventHandler
			while (!(parentClassName.equals(MakerEventHandler.class.getSimpleName())
					|| parentClassName.equals(NpcEventHandler.class.getSimpleName()))) {
				while (!DBClassInfoContainer.getInstance().exists(parentClassName)) {
					try {
				...
				DBClassInfo classInfo = DBClassInfoContainer.getInstance().get(parentClassName);
				...
				parentClassName = classInfo.getParent();
			...
			// Если проперти был найден в родительских классах - делаем override в текущем
			if (found) {
		...
		for (int i = init; i < lines.length; i++) {
			...
			switch (parts[0]) {
				...
				case "push_reg_sp":
					...
					break;
				default:
					//log.warn("Unknown line in LLParser.parseHandlerBody(line):\nToken: {}\nLine:{}", parts[0], line);
					break;
			}
		}
	}
	...
	private int trySwitchCase(Stack<StackObject> stack, int i, String[] lines, AiClass aiClass) {
		if (lines.length > i + 4) {
			...
			if (fetch_i.equals("fetch_i") && push_const.startsWith("push_const") && equal.equals("equal")
				...
				if (hasBreak) {
					if (!caseHandler.stack.isEmpty()) {
						if ((caseHandler.stack.lastElement() instanceof ReturnEvent)) {
							hasBreak = false;
						}
					}
				}
					...
					if (lines[i - 1].startsWith("jump")) {
						...
						for (j = i; j < lines.length; j++) {
							if (lines[j].equals(jumpLabel)) {
								...
								if (tempStack.size() == 0) {
									// do nothing
								} else if (tempStack.get(0) instanceof SubHandlerBody
										&& ((SubHandlerBody) tempStack.get(0)).getType() == SubHandlerBody.IF_TYPE) // else - if
								{
	...
	private int add(Stack<StackObject> stack, int i, String[] lines) {
		...
		if (a1.toString().equals("myself.i_ai0")) {
			log.info("");
		}
	...
	private int func_call(Stack<StackObject> stack, int i, String[] lines, AiClass aiClass) {
		...
		if (!found) {
			if (!notImplementedEvents.contains(eventId)) {
				log.error("LLParser.func_call not found func with eventId=" + eventId);
				notImplementedEvents.add(eventId);
			}
		}
	...
	private TreeSet<ParameterDefine> readParameterDefines(String line, BufferedReader br,
														  AiClass aiClass) throws IOException {
		TreeSet<ParameterDefine> parameterDefines = new TreeSet<>();
		...
		while (!(line = br.readLine()).equals("parameter_define_end")) {
			...
			if (line.startsWith("int")) {
				...
				parameterDefine = new ParameterDefine(String.class, parts[1], str.toString());
			...
			boolean found = false;
			while (!(className.equals(MakerEventHandler.class.getSimpleName()) || className.equals(NpcEventHandler.class.getSimpleName()))) {
				while (!DBClassInfoContainer.getInstance().exists(className)) {
					try {
						Thread.sleep(500);
					} catch (InterruptedException e) {
						e.printStackTrace();
				...
				DBClassInfo classInfo = DBClassInfoContainer.getInstance().get(className);
